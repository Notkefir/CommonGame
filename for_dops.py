...
class Start:
    def __init__(self):
        pygame.init()
        self.fon = pygame.transform.scale(load_image2('fon-game.png'), (800, 600))
    ...
    def new(self):
        self.playing = True
        """потом заменить на pygame.sprite.Group() обязательно"""
    ...
    def game_over(self):
        ...
        restart_btn = ButtonForRest(200, 300, 120, 50, 'WHITE', 'BLACK', 'RESTART', 32)
        ...
        while self.running:
            ...
            if restart_btn.is_pressed(mouse_pos, mouse_pressed):
                SCORE = 0
                ENEMIES_COUNT = 0
                self.new()
                self.main()

            fon = load_image('backgroundfonsettings.jpg')
            self.screen.blit(fon, (0, 0))
            self.screen.blit(text, text_rect)
            self.screen.blit(restart_btn.image, restart_btn.rect)
            self.clock.tick(60)
            pygame.display.update()


class ButtonForRest:
    def __init__(self, x, y, width, height, fg, bg, text, size):
        ...
        self.text = self.font.render(self.text, True, self.fg)
